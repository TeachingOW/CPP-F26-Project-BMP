...
### Description
The flood fill algorithm starts from a given pixel and fills all neigbour pixels of the same color with a new color until it encounters a boundary color. Use a `std::queue`  and a `visited` unordered_set.
...
function floodFill(image, startX, startY, fillColor):
    ...
    Initalize visited as unordred_set
...
## Code
You can use the header file from the class [bmp.hpp](bmp.hpp).
